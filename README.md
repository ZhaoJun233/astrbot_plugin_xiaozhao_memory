...
小昭本地持久记忆插件。插件会在后台记录群聊消息，并在小昭准备回复前注入相关记忆；记忆保留时长由 `retention_hours` 自定义，不再固定为 12 小时。
...
- 同时适配 OneBot 和 QQ Official：OneBot 使用 QQ 号和群号；QQ Official 使用 openid/group_openid，并用 AstrBot 平台实例 ID 稳定区分不同官方机器人。
- 插件只在小昭已经要回复时注入记忆，不会主动触发机器人发言。

## 功能

- 后台记录群聊中的非机器人消息。
- 记忆保留时长可通过 `retention_hours` 自定义，默认配置值为 `12` 小时。
...
| 维度 | 用途 |
| --- | --- |
| `platform_id` | 区分 AstrBot 平台实例，例如 OneBot 实例或 QQ Official 实例。 |
| `bot_id` | 区分当前机器人账号，支持 AstrBot 多机器人隔离。OneBot 使用机器人 QQ 号；QQ Official 归一为平台实例 ID。 |
| `group_id` | 区分群聊，保证群聊记忆不串群。OneBot 是群号；QQ Official 是 `group_openid` 或频道 ID。 |
| `user_id` | 识别同一用户，用于跨群联动。OneBot 是 QQ 号；QQ Official 是 `member_openid`/`user_openid`。 |

实际效果：

- A 群不会读取 B 群的“群聊记忆”。
- 同一个用户在 A 群和 B 群都说过话，小昭回复这个用户时可以看到该用户的跨群相关记忆；在 QQ Official 下这里依赖官方 openid，而不是 QQ 数字号。
- 如果 AstrBot 里启用了两个机器人账号，即使它们在同一个群里，也会使用不同的 `bot_id` 隔离记忆。
- 早期版本写入的 QQ Official 记录如果 `bot_id` 是 `qq_official` 或 `unknown_selfid`，新版本读取时会作为当前官方机器人实例的兼容别名查询；别名仍被限制在同一个 `platform_id` 下，不会跨不同官方机器人实例串库。
...
| `retention_hours` | int | `12` | 自定义记忆保留小时数。插件按该窗口读取和清理旧记忆，默认值可在配置中修改。 |
...
6. 需要记忆时，插件把当前问题提炼成检索关键词，并按当前平台实例、机器人账号、群/会话 ID、用户 ID 检索相关记忆。
...
- 插件启动后需要有新的群消息才会写入。
- `enabled` 必须为 `true`。
- 如果刚切到 QQ Official，注意官方机器人使用 openid/group_openid，不再是 QQ 数字号；这是正常现象，不代表没有记录。
...
- 确认当前回复和历史消息来自同一个机器人实例。数据库会按 `platform_id + bot_id` 隔离不同机器人，避免多个机器人共享同一份记忆。
